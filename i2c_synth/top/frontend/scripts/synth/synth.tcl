...
dc::create_clock [dc::get_ports {clk}] -name "clk" -period $CLK_PERIOD
...
if { $DFT == 1} {
    ...
    define_dft test_clock -name clk -period [expr ($CLK_PERIOD*1000)] clk
	...
	fix_dft_violations -clock -test_control dft_shift_en -test_clock_pin clk
...
if { $DFT == 1} {
    ...
    if {$numDFTviolations > "0"} {
	report dft_violations > ${REPORTSDIR}/${DESIGN}_DFTviols.rpt
	fix_dft_violations -xsource -test_control dft_tm_i -test_clock_pin clk
	fix_dft_violations -clock -test_control dft_tm_i -test_clock_pin clk
...
report area -detail                             > ${REPORTSDIR}/report_area_${DESIGN}.txt
